...
- 状態: 検証実装
- 外部ライブラリ: なし（ブラウザ標準のSVG DOM APIを使用）
- 実装: `frontend/src/features/avatar/svgRenderer.ts`
- 髪素材: `frontend/src/features/avatar/assets/hair/*.svg`
- 顔追跡: 既存のMediaPipe `FaceLandmarker`から得た値を使用

顔、髪、目、口、服をSVGのグループとして分け、毎フレームSVG全体を作り直さず、
各パーツの`transform`と寸法だけを更新する。勉強中は発話を想定しないため口は
ニュートラル表示で固定し、頭の下降、顔パーツの下降、視線とまぶたを組み合わせて
ノートなどを見下ろす動作を表現する。Canvas版より輪郭や陰影を
調整しやすく、小さなサブウィンドウでも視認性を保ちやすい。外部アバター素材や
専用ランタイムを配布しないため、Live2D方式より容量とライセンス管理を抑えられる。

髪型はレンダラー内へパスを直接記述せず、1髪型につき1つのSVG素材として管理する。
各素材には`back`、`crown`、`foreground`の3つの`symbol`を定義する。後頭部を顔より
後ろ、頭頂部を顔の上、前髪を眉や額より前へ描画することで、短髪、センターパート、
マッシュ、アップバングのシルエットと重なり方を分ける。素材側は顔追跡に依存せず、
レンダラーが3レイヤーへ同じ頭部変形を適用する。
